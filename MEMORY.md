...
2. **Intelligence triad boundary contract is P0.** `dev-nexus/docs/architecture/architectural-boundaries.md` (dated 2026-06-09). Forcing function: "If a PR adds code to X that could be answered by Y, it belongs in Y." Treat this as the spec for the whole triad. **Triad wiring status (audited 2026-06-24):** only `openclaw-gateway → mcp-tooling` is wired at runtime. `openclaw-gateway → dev-nexus` is *declared* (A2A + MCP interfaces defined in the boundary doc, no import evidence in either repo's source). `dev-nexus → mcp-tooling` is *doc-references only* (`mcp-tooling/runtime/base.py:4-9` says `BaseTool` shape mirrors `BaseSkill` — parallel design, not a runtime call). The triad is a governance structure with one runtime integration; "the intelligence triad" implies more wiring than currently exists.
3. **`openclaw-gateway` has the strongest env hygiene in the org** — DAT contract (no hardcoded env values), explicit test/head/prod topology, one bot per env. Model other repos should follow. **The "no hardcoded env values" principle is org-wide** per `.github/docs/CI-CD-STANDARDS.md` §1 "No Hardcoded Values" (resolved 2026-06-14, Q8). `openclaw-gateway` calls it the "DAT contract"; the org standard calls it the same thing under a different name.
...
12. **Cross-project IAM for shared-DB / shared-secret patterns (resolved 2026-08-11, load-bearing).** When one repo owns a secret (or DB) in GCP project A and another repo's service in project B needs to read it: (1) the IAM binding lives in **project A's terraform** (which may be a *third* repo, e.g. `gcp-postgres-terraform`, not in either PR repo); (2) the consumer references the secret by full resource name (`projects/<A>/secrets/<id>`) passed as a `var.<name>_secret_resource_name`; (3) the consumer's terraform can take a `var.db_secret_iam_bindings` map and emit `google_secret_manager_secret_iam_member` resources with `roles/secretmanager.secretAccessor`; (4) **apply order is IAM first**, verify with `gcloud secrets get-iam-policy`, then consumer deploy, then any terraform cleanup that drops the duplicate. The org already uses this pattern in `openclaw-gateway` and `terraform-linux-desktop` (see `dev-nexus/docs/architecture/architectural-boundaries.md` §3.2 "Cross-project secret reads"). First live cross-product consolidation: research-orchestrator → rag_research_tool Postgres (PR #42, issue #1354). **Rule for cross-repo RFCs that involve shared infrastructure:** the implementation PR may need to land in a third repo. Surface this in the RFC body before the implementation PR is opened.
