...
- **Verify repo cwd before every git op in multi-repo sessions.** The 2026-08-11 incident: I ran a parallel `exec` block touching multiple cloned repos in `_context/`. `git add -A && git commit` ran from `darojaai_architect/main` instead of `intelligent-feed/chore/activators-http-ingest` because both blocks inherited the workspace cwd. Push was rejected by branch protection so nothing actually landed, but I had to `git reset --hard HEAD~1` to clean up — and I made the same mistake a second time. **Rule:** when working in `_context/<repo>/`, start every `exec` block with `cd /home/desktopuser/GithubProjects/darojaai_architect/_context/<repo> && pwd && git rev-parse --abbrev-ref HEAD` before any `git add` / `git commit` / `git push`. Verify the branch *after* the push too. Never use `git add -A` — use `git add <explicit-paths>` so a wrong cwd cannot silently pick up unrelated files. **Cost of the verify step:** one tool call. **Cost of skipping it:** a corrupted `main` on my own repo and operator trust damage.
- **Don't promise PR self-approval as a deliverable.** GitHub blocks author self-approval of own PRs. `gh pr review --approve` on a PR I authored fails with "Review Can not approve your own pull request" (GraphQL error). The 2026-08-11 incident: I told the operator I would self-approve both PRs, then the tool failed and I had to acknowledge the misframing. **Rule:** the closeout of a green PR is "CI green, ready for your approval" — not "I will approve it." Surface status, ask the operator to approve, accept the answer "already merged" or "skip approval" gracefully.
- **Ruff-version drift between local and CI.** When local ruff ≠ CI-pinned ruff, the formatter can disagree and `pre-commit hooks` will fail with "files were modified by this hook" even though my local run was clean. `pip install ruff==<pinned>` in the same venv does **not** always update the binary (shadowed install, venv isolation, etc.). The 2026-08-11 incident: I burned ~15 min on `pip install ruff==0.4.10` before checking that the binary was still 0.15.17, then read the CI diff and applied the expected format manually. **Rule:** when CI's pinned version differs from local, either (a) use a fresh venv (`python3 -m venv /tmp/ruff-pinned && /tmp/ruff-pinned/bin/pip install ruff==<pinned>`) so the binary is guaranteed fresh, or (b) read the CI diff verbatim and apply the format by hand. Don't trust `pip install` against an existing venv to fix it. The CI run ID link is in the PR checks output — read it.
